...
/**
 * 客户端心跳，没有用到，保证接入正确返回就行
 * 
 * @author huangbuji huangbuji@megaeyes.com
 *         <p />
 *         2012-7-20 上午09:17:57
 */
public class HeartBeatAction extends BaseAction implements Action {

	private static final Log log = LogFactory.getLog(HeartBeatAction.class);

	public Document proccess(RequestObject req) throws BusinessException {
		log.info("In HeartBeatAction...");
		// 返回
		Document rtn = new Document();
		Element rtnRoot = new Element("Message");
		rtn.setRootElement(rtnRoot);

		Element expireElement = new Element("IsExpire");
		expireElement.setText("0");
		rtnRoot.addContent(expireElement);
